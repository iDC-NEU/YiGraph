...
def count_files_in_knowledge_base(kb_id):
    ...
    try:
        ...
        logger.warning(f"统计知识库 {kb_id} 文件个数失败: {e}")
        return 0
...
def delete_knowledge_base(kb_id):
    ...
    if kb_to_delete:
        ...
        kb_folder_path = os.path.join(
            FILE_STORAGE_BASE_PATH, kb_to_delete.get("name") or kb_to_delete.get("名称")
        )
        try:
            import shutil
