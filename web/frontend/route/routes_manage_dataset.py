...
def api_check_parsing_status():
    ...
    try:
        ...
        if not kb_id:
            return jsonify({"success": False, "error": "Missing kb_id"}), 400

        kb_name = get_knowledge_base_name(kb_id)
        kb_type = get_dataset_type_from_back(kb_id)
        # 获取解析状态
        a24 = DummySocket(
            json.dumps({"action": "get_parsing_status", "ds_name": kb_name})
        )
        _run_async(server_Test.handler(a24))
        acps = a24.returnmsg
        acpss = json.loads(acps)
        if kb_type == "graph":
            a25 = DummySocket(
                json.dumps({"action": "get_dataset_schema", "ds_name": kb_name})
            )
            _run_async(server_Test.handler(a25))
            acps5 = a25.returnmsg
            acpss5 = json.loads(acps5)
            if acpss5["content"]["data"][0].get("vertex_file", None) is not None:
                acpss["content"]["data"]["parsing_status"][
                    acpss5["content"]["data"][0]["vertex_file"].split("/")[-1]
                ] = "completed"
        return jsonify(
            {
                "success": True,
...
def load_knowledge_bases():
    ...
    logger.debug(f"load_knowledge_bases 原始返回: {knowledge_bases_with_count}")
...
def api_preview_file():
    ...
    try:
        ...
        if not kb_id or not file_name:
            return jsonify(
                {"success": False, "error": "Missing kb_id or file_name"}
            ), 400
        ...
        if kb_type == "text":
            ...
            if file_name in server_Test.each_dataset:
                file_path = server_Test.each_dataset[file_name]["schema"].get(
                    "original_path"
                )
        elif kb_type == "graph":
            ...
            if not os.path.isabs(server_Test.dataset_folder):
                ...
                dataset_folder_abs = os.path.normpath(
                    os.path.join(api_dir, server_Test.dataset_folder)
                )
            else:
            ...
            for dataset_name, dataset_info in server_Test.each_dataset.items():
                ...
                def resolve_file_path(original_path, target_file_name):
                    ...
                    if (
                        target_file_name != path_file_name
                        and target_file_name not in original_path
                    ):
                        return None
                    ...
                    if not os.path.isabs(original_path):
                        ...
                        if "data/" in original_path:
                            ...
                            rel_path = original_path[
                                data_index:
                            ]  # data/{kb_name}/graph/{file_name}
                            resolved_path = os.path.normpath(
                                os.path.join(dataset_folder_abs, rel_path)
                            )
                        else:
                            # 如果没有data/，直接使用标准路径
                    ...
                    if os.path.exists(resolved_path):
                        ...
                        graph_dir = os.path.join(
                            dataset_folder_abs, "data", kb_name, "graph"
                        )
        ...
        if not file_path or not os.path.exists(file_path):
            # 添加详细的错误信息用于调试
            logger.error(
                f"文件未找到: kb_id={kb_id}, kb_name={kb_name}, file_name={file_name}, file_path={file_path}"
            )
            if kb_type == "graph" and "dataset_folder_abs" in locals():
                logger.error(f"dataset_folder_abs: {dataset_folder_abs}")
                # 列出可能的路径
                possible_path = os.path.join(
                    dataset_folder_abs, "data", kb_name, "graph", file_name
                )
                logger.error(
                    f"尝试的标准路径: {possible_path}, 存在: {os.path.exists(possible_path)}"
                )
                # 列出graph目录下的所有文件
        ...
        if file_extension == "csv":
            ...
            try:
                ...
                try:
                    ...
                    content = {
                        ...
                        "rows": rows,
                        "total_rows": sum(
                            1 for _ in open(file_path, "r", encoding="gbk")
                        )
                        - 1,
                        "displayed_rows": len(rows),
                    }
                ...
                logger.error(f"读取CSV文件失败: {str(e)}")
                return jsonify(
                    {"success": False, "error": f"Failed to read CSV file: {str(e)}"}
                ), 500
            ...
            try:
                ...
                return jsonify(
                    {"success": False, "error": f"Failed to read PDF file: {str(e)}"}
                ), 500
                ...
                try:
                    ...
                    if hasattr(result, "html_content") and result.html_content:
                        ...
                        content = markdown.markdown(
                            result.text_content, extensions=["tables", "fenced_code"]
                        )
                    content_type = "html"
                except Exception as e:
            ...
            return jsonify(
                {"success": False, "error": f"Unsupported file type: {file_extension}"}
            ), 400
    ...
    try:
        ...
        if is_batch_upload and file_type == "graph":
            ...
            for file in files:
                ...
                saved_files.append(
                    {"filename": file.filename, "path": file_path, "size": file_size}
                )
                ...
                for saved_file in saved_files:
                    if os.path.exists(saved_file["path"]):
                        try:
                            ...
                            logger.error(
                                f"删除文件失败 {saved_file['path']}: {str(del_e)}"
                            )
                return jsonify(
                    {"success": False, "error": f"Invalid graph_info JSON: {str(e)}"}
                ), 400
            graph_name = graph_info.get("graphName")
            # 提取顶点文件相关字段
            vertex_file_name = graph_info["vertexSchema"].get(
                "fileName", ""
            )  # 顶点文件名（可选）
            vertex_id_field = graph_info["vertexSchema"].get(
                "idField", ""
            )  # 顶点ID字段（如果有顶点文件则必填）
            vertex_name_field = graph_info["vertexSchema"].get("nameField", "")
            vertex_name_field = (
                vertex_id_field if not vertex_name_field else vertex_name_field
            )  # 顶点名称字段（如果有顶点文件则必填）

            # 提取边文件相关字段
            edge_file_name = graph_info["edgeSchema"]["fileName"]  # 边文件名（必填）
            edge_source_field = graph_info["edgeSchema"][
                "sourceField"
            ]  # 源节点字段（必填）
            edge_target_field = graph_info["edgeSchema"][
                "targetField"
            ]  # 目标节点字段（必填）
            edge_relation_field = graph_info["edgeSchema"].get(
                "relationField", ""
            )  # 关系字段（必填）
            weight_col = graph_info["edgeSchema"].get(
                "weightField", ""
            )  # 权重字段（可选）
            is_directed = graph_info["graphProperties"].get(
                "isDirected", True
            )  # 是否为有向图
            vertex_attribute_field = format_string_to_list(
                graph_info["vertexSchema"].get("propertiesField", "")
            )  # 顶点属性字段（可选），转为后端期望的列表字面量字符串
            vertex_query_field = graph_info["vertexSchema"].get(
                "nameField", ""
            )  # 顶点查询字段（可选）
            # 验证必填字段
            ...
            msg111 = json.dumps(
                {
                    ...
                    "ds_name": kb_name,
                    "vertex_file_name": vertex_file_name
                    if vertex_file_name
                    else None,  # 只有提供了文件名才发送
                    "vertex_id_field": vertex_id_field if vertex_file_name else None,
                    "vertex_name_field": vertex_name_field
                    if vertex_file_name
                    else None,
                    ...
                    for saved_file in saved_files:
                        if os.path.exists(saved_file["path"]):
                            try:
                                ...
                                logger.error(
                                    f"删除文件失败 {saved_file['path']}: {str(e)}"
                                )
                return jsonify({"success": False, "error": error_content}), 400
            ...
            if file_type == "text":
                ...
                try:
                    ...
                    if saved_file_path and os.path.exists(saved_file_path):
                        try:
                            ...
                            logger.error(
                                f"删除文件失败 {saved_file_path}: {str(del_e)}"
                            )
                    return jsonify(
                        {
                            "success": False,
                ...
                vertex_name_field = graph_info["vertexSchema"].get(
                    "nameField", ""
                )  # 顶点名称字段（如果有顶点文件则必填）

                # 提取边文件相关字段
                edge_file_name = graph_info["edgeSchema"][
                    "fileName"
                ]  # 边文件名（必填）
                edge_source_field = graph_info["edgeSchema"][
                    "sourceField"
                ...
                msg111 = json.dumps(
                    {
                        ...
                        "vertex_id_field": vertex_id_field
                        if vertex_file_name
                        else None,
                        "vertex_name_field": vertex_name_field
                        if vertex_file_name
                ...
                if acps["type"] == "error":
                    if os.path.exists(file_path):
                        try:
                            ...
                            logger.error(f"删除文件失败 {file_path}: {str(e)}")
                    return jsonify(
                        {"success": False, "error": acps.get("content", "上传失败")}
                    ), 400
            ...
            for saved_file in saved_files:
                if os.path.exists(saved_file["path"]):
                    try:
                        ...
                        logger.error(
                            f"异常处理：删除文件失败 {saved_file['path']}: {str(del_e)}"
                        )
        # 单个文件上传的情况
            ...
            if kb.get("file_type") == "graph" and kb.get("file_count") == 1:
                try:
                    ...
                    if (
                        ...
                        and data_list
                        and data_list[0].get("vertex_file", None) is not None
                    ):
                        kb["file_count"] = 2
...
def get_dataset_type():
    ...
    try:
        ...
        return jsonify({"success": True, "dataset_type": dataset_type})
    except Exception as e:
        logger.error(f"获取数据集类型失败: {str(e)}")
        return jsonify({"success": False, "error": f"Invalid kb_id: {raw_kb_id}"}), 500
...
def api_delete_file():
    ...
    try:
        ...
        if kb_type == "graph":
            ...
            _run_async(server_Test.handler(a26))
            acps = a26.returnmsg
            acps = json.loads(acps)
            file_name = acps["content"]["data"][0]["name"]
        try:
            socketserver2 = DummySocket(
                json.dumps(
                    {
                        "action": "delete_file",
                        "file_name": file_name,
            ...
            _run_async(server_Test.handler(socketserver2))
            ...
            return jsonify(
                {"success": False, "error": f"Failed to delete directory: {str(e)}"}
            ), 500
...
def api_parse_control():
    ...
    try:
        ...
        if not kb_id or not file_name or not action:
            return jsonify(
                {"success": False, "error": "Missing kb_id, file_name or action"}
            ), 400
        ...
        socketserver = MessageCollectingSocket(
            json.dumps(
                {
                    "action": "parsing_file",
                    "file_name": file_name,
        ...
        _run_async(server_Test.handler(socketserver))
...
def api_generate_graph():
    ...
    try:
        ...
        if dataset_type == "text":
            ...
            msg12 = json.dumps(
                {
                    "action": "get_overall_triplets",
                    # "graph_name": graph_name1,
                    "ds_name": kb_name,
                }
            )
            a33 = DummySocket(msg12)
            _run_async(server_Test.handler(a33))
            agg = a33.returnmsg
            agg1 = json.loads(agg)
            ...
            msg12 = json.dumps(
                {
                    "action": "get_file_triplets_from_graph_dataset",
                    "file_name": files[0],
                    "ds_name": kb_name,
                }
            )
