...
from graphllm.pipeline import GraphLLMPipeline, create_pipeline_config


def example_basic_usage():
    """基础使用示例"""
    print("=== GraphLLM Pipeline 基础使用示例 ===")
    ...
    # 2. 初始化Pipeline
    pipeline = GraphLLMPipeline(config)
    ...
    for i, question in enumerate(questions, 1):
        ...
        # 使用图检索和向量检索
        result = pipeline.query(question, use_graph=True, use_vector=True)
...
def example_advanced_usage():
    ...
    result_graph_only = pipeline.query(question, use_graph=True, use_vector=False)
    print(f"问题: {question}")
    ...
    # 示例：只使用向量检索
    print("\n--- 仅向量检索示例 ---")
    result_vector_only = pipeline.query(question, use_graph=False, use_vector=True)
    print(f"问题: {question}")
    ...
    # 示例：混合检索
    print("\n--- 混合检索示例 ---")
    result_hybrid = pipeline.query(question, use_graph=True, use_vector=True)
    print(f"问题: {question}")
