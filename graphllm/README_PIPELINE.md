...
# 查询
result = pipeline.query("What are Graph Neural Networks?")
print(result['answer'])
```
...
```python
# 仅使用图检索
result = pipeline.query(question, use_graph=True, use_vector=False)

# 仅使用向量检索
result = pipeline.query(question, use_graph=False, use_vector=True)
...
try:
    pipeline = GraphLLMPipeline(config)
    result = pipeline.query("What is GNN?")
...
# 在Pipeline中集成
pipeline.custom_retriever = CustomRetriever()
```
...
```python
# 在Pipeline中添加自定义指标
pipeline.metrics["custom_metric"] = []
...
# 在Pipeline中启用调试
config = create_pipeline_config(verbose=True)
```
