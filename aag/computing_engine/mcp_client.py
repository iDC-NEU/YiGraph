...
class GraphMCPClient:
        ...
        if self.schema_dir:
            ...
            if len(self.server_args) >= 2 and self.server_args[0] == "-m":
                ...
                schema_file = (
                    ...
                    / module_dir
                    / f"generated_{schema_type}_schemas.json"
                )
            else:
                schema_file = (
                    Path(self.server_args[0]).parent
                    / f"generated_{schema_type}_schemas.json"
                )
        else:
            schema_file = (
                Path(__file__).parent / f"generated_{schema_type}_schemas.json"
            )
        ...
        try:
            ...
            for tool in tools_response.tools:
                ...
                if not input_schema:
                    ...
                    logger.debug(
                        f"⚠️ Tool '{tool_name}' using server-provided input_schema"
                    )
    ...
    def get_tool_schema(
        self, tool_name: str, schema_type: str = "input"
    ...
    def validate_arguments(
        ...
        for param_name in arguments.keys():
            if (
                ...
                and param_name != "__post_processing_code__"
            ):
        ...
        try:
            ...
            for key, value in arguments.items():
                ...
                if isinstance(value, (dict, list)) and len(value) == 0:
                    ...
                    logger.warning(
                        f"⚠️ Parameter '{key}' is an empty container, converted to None"
                    )
                else:
            ...
            result = await self.session.call_tool(
                tool_name, arguments=cleaned_arguments
            )
            if result.content and len(result.content) > 0:
                ...
                if hasattr(content, "text"):
                    try:
                        ...
                        logger.error(
                            f"JSON parsing failed: {e} - Raw response: {raw_content}"
                        )
                        return {
                            "success": False,
                            "error": raw_content,
                    ...
                    return {
                        "success": False,
                        "error": "Tool call failed: Unable to parse tool call result",
                    }
            else:
                return {
                    "success": False,
            ...
            if post_processing_code:
                ...
                try:
                    ...
                    original_response["summary"] = (
                        original_response.get("summary", "")
                        + " (Local post-processing applied)"
                    )
                    ...
                    return {
                        "success": False,
                        "error": f"❌ Post-processing code execution failed:  {post_error}",
                    }
            ...
            return {"success": False, "error": str(e), "summary": "Tool call error"}
    ...
    # TODO: load_data_from_csv() 放在 MCP 客户端类中违反单一职责原则。
    #       该方法应迁移至 aag.data_pipeline 模块（如 CsvDataLoader）。
