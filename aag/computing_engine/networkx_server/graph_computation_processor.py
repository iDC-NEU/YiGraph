...
class GraphComputationProcessor(GraphProcessor):
    ...
    def __init__(self):
        ...
        # Louvain 社区检测结果缓存：避免同一图上重复计算
        # 键为 (节点数, 边数, resolution, random_state) 元组，值为结果字典
        self._louvain_cache: Dict[
            Tuple[int, int, float, Optional[int]], Dict[str, Any]
        ] = {}
    ...
    def create_graph_from_edges(
        ...
        try:
            ...
            logger.debug(
                f"✅ 图构建完成: {self.graph.number_of_nodes()} 个节点，{self.graph.number_of_edges()} 条边"
            )
            return self.graph
    ...
    def run_shortest_path(self, source: Any, target: Any) -> Optional[List[Any]]:
        ...
        try:
            # TODO: networkx.shortest_path 对 Directed/Undirected 图自动适配；
            # 若未来需要区分算法（如 Dijkstra/Bellman-Ford），在此补充有向图专属逻辑。
    ...
    def run_betweenness_centrality(self) -> Dict[Any, float]:
        ...
        try:
            ...
            logger.info(
                f"介数中心性计算完成，计算了 {len(betweenness_scores)} 个节点的分数"
            )
    ...
    def run_closeness_centrality(self) -> Dict[Any, float]:
        ...
        try:
            ...
            logger.info(
                f"接近中心性计算完成，计算了 {len(closeness_scores)} 个节点的分数"
            )
    ...
    def run_louvain_community_detection(
        self, resolution: float = 1.0, random_state: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        使用Louvain算法进行社区检测
        ...
        if cache_key in self._louvain_cache:
            logger.debug(
                f"Louvain社区检测命中缓存 (nodes={cache_key[0]}, edges={cache_key[1]})"
            )
            return self._louvain_cache[cache_key]
        ...
        try:
            ...
            partition = community.best_partition(
                undirected_graph, resolution=resolution, random_state=random_state
            )
            ...
            logger.info(
                f"Louvain社区检测完成，发现 {len(community_list)} 个社区，模块度: {modularity:.4f}"
            )
            # 存入缓存，后续相同图+参数可直接复用
            self._louvain_cache[cache_key] = result
            return result
    ...
    def get_community_by_specific_id(
        self, query_vertex_id: Optional[int] = None
    ) -> Dict[str, Any]:
        """
        获取指定节点ID的社区结果
        ...
        if query_vertex_id is None:
            raise ValueError(
                "未指定查询节点ID，请设置query_vertex_id参数或self.query_vertices"
            )
        ...
        result = {
            ...
            "total_neighbors": len(neighbors_in_community)
            + len(neighbors_outside_community),
            "community_cohesion": len(neighbors_in_community)
            / max(1, len(all_neighbors))
            if "all_neighbors" in locals()
            else 0.0,
        }

        logger.info(
            f"节点 {query_vertex_id} 的社区信息: 社区ID={community_id}, 社区大小={len(community_members)}"
        )

        return result

    def _calculate_modularity(
        self, graph: nx.Graph, partition: Dict[Any, int]
    ) -> float:
        """
        计算模块度
    ...
    def get_community_statistics(
        self, louvain_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        获取社区检测的统计信息
    ...
    def run_algorithm(self, algorithm: str, **kwargs) -> Union[Dict, List, Any]:
        ...
        if algorithm not in algorithm_map:
            raise ValueError(
                f"不支持的算法: {algorithm}。支持的算法: {list(algorithm_map.keys())}"
            )
    ...
    def get_graph_info(self) -> Dict[str, Any]:
        ...
        info = {
            ...
            "是否强连通": nx.is_strongly_connected(self.graph)
            if self.is_directed
            else None,
        }
    ...
    def execute_plan(
        self, edges: List[Tuple], plan: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        根据执行计划按顺序执行算法
