...
from aag.computing_engine.graph_query.nl_query_engine_refactored import (
    ParameterExtractorRouter,
)
# add gjq
# 已移除直接初始化 OpenAI 客户端的硬编码配置
# 原 API Key 和 Base URL 已从环境变量读取，请勿在此处回填敏感信息
# os.environ['OPENAI_API_KEY'] = '<从环境变量读取>'
# os.environ['OPENAI_BASE_URL'] = '<从环境变量读取>'
# client = OpenAI(
...
class QueryTypeClassifier:
    ...
    def classify(self, question: str) -> str:
        ...
        if query_type == "subgraph_extract_by_nodes":
            logging.warning(
                f"LLM 返回了错误的类型名 '{query_type}'，自动修正为 'subgraph_by_nodes'"
            )
    ...
    def _fallback_classify(self, question: str) -> str:
        ...
        if any(kw in q for kw in ["统计每个", "计算每个", "每个", "各个", "分别"]):
            ...
            if has_specific_node:
                logging.info(
                    f"检测到具体节点 + '每个'关键词，使用 neighbor_query + aggregate"
                )
        ...
        if any(
            ...
            if has_specific_node:
                logging.info(
                    f"检测到具体节点 + 聚合关键词，使用 neighbor_query + aggregate"
                )
                return "neighbor_query"

            # 排除"找出"、"列出"、"返回"等返回列表的关键词
            if not any(
                kw in q for kw in ["找出", "列出", "返回...的交易", "查询...的交易"]
            ):
                return "aggregation_query"
        ...
        if "公共" in q or "共同" in q or ("既" in q and "又" in q):
            ...
            if any(kw in q for kw in ["为true", "为false"]):
                logging.warning(
                    f"检测到复杂的节点属性条件公共邻居查询，当前模板不支持，建议使用自定义Cypher"
                )
        ...
        if "子图" in q or any(
            kw in q for kw in ["包含节点", "节点列表", "相互之间", "连边"]
        ):
            # 检测是否包含多个节点标识
...
class CypherValidator:
    ...
    def __init__(
        self, reasoner: Reasoner, schema_analyzer: Optional["SchemaAnalyzer"] = None
    ):
        self.reasoner = reasoner
        self.schema_analyzer = schema_analyzer

    def validate(
        self, cypher: str, question: str, query_type: str, params: Dict
    ) -> Dict:
        """
    ...
    def execute(self, query_type: str, params: Dict) -> Dict:
        ...
        try:
            ...
            if query_type == "node_lookup":
                ...
                # 聚合统计查询
                # 检查是否有分组参数
                has_grouping = params.get("group_by_node") or params.get(
                    "group_by_property"
                )
                ...
                if not has_grouping:
                    logging.warning(
                        "检测到 aggregation_query 缺少分组参数，转换为全局聚合查询"
                    )
                    ...
                    if params.get("conditions"):
                        ...
                        if agg_type == "COUNT":
                            results = [
                                {"aggregate_type": "count", "value": len(results)}
                            ]
                        else:
                            ...
                            if agg_field:
                                results = self._apply_aggregation(
                                    results, agg_type, agg_field
                                )
                            else:
                                results = [
                                    {
                                        "error": f"聚合类型 {agg_type} 需要指定 aggregate_field"
                                    }
                                ]
                    ...
                    # 场景2：关系属性聚合（有 rel_type）
                        ...
                        logging.info(
                            f"aggregate_type: {agg_type}, aggregate_field: {agg_field}"
                        )

                        # ⚠️ CRITICAL: 确保 aggregate_field 正确传递
                        # 对于 SUM/AVG/MAX/MIN，aggregate_field 是必需的
                        if agg_type and agg_type.upper() in [
                            "SUM",
                            "AVG",
                            "MAX",
                            "MIN",
                            ...
                            if not agg_field:
                                ...
                                if self.schema_analyzer:
                                    ...
                                    rel_info = schema.get("relationship_types", {}).get(
                                        params["rel_type"], {}
                                    )
                                    ...
                                    for common_field in [
                                        ...
                                        if common_field in rel_props:
                                            ...
                                            logging.warning(
                                                f"aggregate_field 未指定，自动推断为: {agg_field}"
                                            )
                                            break
                                    ...
                                    if not agg_field:
                                        ...
                                        for prop in rel_props:
                                            if prop not in [
                                                ...
                                                agg_field = prop
                                                logging.warning(
                                                    f"aggregate_field 未指定，使用第一个可能的数值字段: {agg_field}"
                                                )
                                                break
                                else:
                                    logging.error(
                                        "schema_analyzer 未初始化，无法自动推断 aggregate_field"
                                    )
                        ...
                        results = [
                            {
                                "error": "aggregation_query 缺少必要参数：需要 group_by_node/group_by_property 或 conditions 或 rel_type"
                            }
                        ]
                ...
                if aggregate_type:
                    results = self._apply_aggregation(
                        results, aggregate_type, aggregate_field
                    )
                ...
                if "rel_conditions" in params and params["rel_conditions"]:
                    ...
                    enable_aggregate = (
                        aggregate_type and aggregate_type.lower() == "count"
                    )
                ...
                if aggregate_type:
                    if aggregate_type.lower() == "count":
                        ...
                        # 其他聚合类型（如 SUM、AVG 等）
                        results = self._apply_aggregation(
                            [result], aggregate_type, aggregate_field
                        )
                else:
                    results = [result]
...
class NaturalLanguageQueryEngine:
    ...
    def ask(self, question: str) -> Dict:
        ...
        try:
            ...
            if "查询类型判断错误" in error_msg and "neighbor_query" in error_msg:
                ...
                try:
                    params = self.param_extractor.extract(question, query_type)
                    print(
                        f"   提取参数: {json.dumps(params, ensure_ascii=False, indent=2)}"
                    )
        ...
        if (
            ...
            and self.cypher_validator
            and query_type not in skip_validation_types
        ):
            print("\n🔍 LLM3 正在验证参数...")
    ...
    def _validate_params(self, question: str, query_type: str, params: Dict) -> Dict:
        ...
        # 调用验证器
        return self.cypher_validator.validate(
            cypher_description, question, query_type, params
        )
    ...
    def _build_cypher_description(self, query_type: str, params: Dict) -> str:
        ...
        if query_type == "common_neighbor":
            description += "\n预期行为:\n"
            description += (
                f"  1. 找出节点 {params.get('v1')} 和 {params.get('v2')} 的公共邻居\n"
            )

            if "rel_conditions" in params and params["rel_conditions"]:
                description += (
                    f"  2. 对两条关系都应用过滤条件: {params['rel_conditions']}\n"
                )
...
def main():
    ...
    # 数据库配置
    uri = (
        input("\nNeo4j URI (默认 bolt://localhost:7687): ").strip()
