...
class Neo4jGraphLoader:
    ...
    def _build_node_key(
        self, row: pd.Series, query_fields: Union[str, List[str]]
    ) -> str:
        """
        ...
        try:
            with self.driver.session() as session:
                for vertex_config in vertex_configs:
                    ...
                    try:
                        session.run(
                            f"CREATE INDEX {label.lower()}_key_index IF NOT EXISTS FOR (n:{label}) ON (n.node_key)"
                        )
                    ...
                    if id_field:
                        try:
                            session.run(
                                f"CREATE INDEX {label.lower()}_id_index IF NOT EXISTS FOR (n:{label}) ON (n.{id_field})"
                            )
        ...
        try:
            ...
            if isinstance(query_field, list):
                df["node_key"] = df.apply(
                    lambda row: self._build_node_key(row, query_field), axis=1
                )
            else:
            ...
            fields_to_load = self._get_attribute_fields(
                df, attribute_fields, exclude_fields
            )
            ...
            with self.driver.session() as session:
                for i in range(0, total_vertices, batch_size):
                    ...
                    try:
                        ...
                        logger.info(
                            f"已处理 {min(i + batch_size, total_vertices)}/{total_vertices} 条{vertex_type}数据"
                        )
                    except Exception as e:
            ...
            logger.info(
                f"{vertex_type}数据加载完成，成功处理 {success_count} 条，失败 {fail_count} 条"
            )
            return True
    ...
    def load_edges(
        self, edge_config: Dict[str, Any], vertex_configs: List[Dict[str, Any]]
    ):
        """
        根据配置加载边数据
        ...
        try:
            ...
            if isinstance(vertex_query_field, list):
                vertex_df["node_key"] = vertex_df.apply(
                    lambda row: self._build_node_key(row, vertex_query_field), axis=1
                )
            else:
            ...
            id_to_node_key = dict(
                zip(vertex_df[vertex_id_field], vertex_df["node_key"])
            )
            ...
            if missing_source > 0 or missing_target > 0:
                logger.warning(
                    f"发现 {missing_source} 个源节点和 {missing_target} 个目标节点无法映射到node_key，这些边将被跳过"
                )
                # 过滤掉无法映射的边
            ...
            with self.driver.session() as session:
                for i in range(0, total_edges, batch_size):
                    ...
                    try:
                        ...
                        logger.info(
                            f"已处理 {min(i + batch_size, total_edges)}/{total_edges} 条{edge_type}数据，成功创建 {batch_success} 条关系"
                        )
                    except Exception as e:
            ...
            logger.info(
                f"{edge_type}数据加载完成，成功插入 {success_count} 条关系，失败 {fail_count} 条"
            )
            return True
    ...
    def verify_data(
        self, vertex_configs: List[Dict[str, Any]], edge_configs: List[Dict[str, Any]]
    ):
        """验证数据加载结果"""
        if not self.driver:
        ...
        try:
            with self.driver.session() as session:
                ...
                for vertex_config in vertex_configs:
                    ...
                    result = session.run(
                        f"MATCH (n:{vertex_type}) RETURN count(n) as count"
                    )
                ...
                if vertex_configs:
                    ...
                    result = session.run(
                        f"MATCH (n:{vertex_type}) RETURN n.node_key, {field_names} LIMIT 5"
                    )
                ...
                if edge_configs and vertex_configs:
                    ...
                    for record in result:
                        logger.info(
                            f"  {record['source_key']} -> {record['target_key']}"
                        )
...
def load_from_yaml(
    ...
    try:
        ...
        loader = Neo4jGraphLoader(
            uri=uri, username=username, password=password, schema=schema
        )
...
def main():
    ...
    #     uri='bolt://202.199.13.67:7687',
    #     username='neo4j',
    #     password=os.getenv("NEO4J_PASSWORD", ""),  # 从环境变量读取密码
    #     clear_existing=True
    # )
