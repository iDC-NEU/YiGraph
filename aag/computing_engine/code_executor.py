...
# 安全内置函数白名单（用于 exec() 沙箱，禁止 __import__/open/eval/exec 等危险函数）
SAFE_BUILTINS = {
    "len": len,
    "sorted": sorted,
    "reversed": reversed,
    ...
    def execute(
        ...
        if isinstance(data, dict) and data.get("type") == "graph":
            try:
                ...
                logger.info(
                    f"🔄 已将序列化的图字典转换回 NetworkX {graph_type} 对象 (节点数: {len(nodes)}, 边数: {len(edges)})"
                )
                data = restored_graph
            except Exception as e:
                logger.warning(
                    f"⚠️  无法将序列化的图字典转换回 NetworkX 图对象: {e}，将使用原始字典"
                )

        # 创建独立命名空间（使用安全 builtins 白名单）
        namespace = {
            "data": data,
