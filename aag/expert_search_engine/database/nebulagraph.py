...
class NebulaClient:
    ...
    def show_triplets(self, space_name, file_path=None):
        ...
        print(f"after filter the sample triplet last {len(all_triples)} triplets.")
...
class NebulaGraphClient:
    ...
    def create_tag(self, tag_name, fields):
        ...
        resp = self.session.execute(
            f"CREATE TAG IF NOT EXISTS {tag_name}({fields_str})"
        )
        if not resp.is_succeeded():
    ...
    def create_edge_type(self, edge_name, fields):
        ...
        fields_str = ", ".join([f"{name} {ftype}" for name, ftype in fields])
        resp = self.session.execute(
            f"CREATE EDGE IF NOT EXISTS {edge_name}({fields_str})"
        )
        if not resp.is_succeeded():
    ...
    def insert_edge(
        self, edge_name, src_vid, dst_vid, prop_names, prop_values, rank=None
    ):
        """
        prop_names: list of property names
        prop_values: list of property values (顺序与prop_names一致)
...
class NebulaDB:
    ...
    def init_nebula_store(self):
        ...
        storage_context = StorageContext.from_defaults(graph_store=nebula_store)
    ...
    def get_rel_map(self, entities, depth=2, limit=30):
        rel_map: Optional[Dict] = self.store.get_rel_map(
            entities, depth=depth, limit=limit
        )
        return rel_map
    ...
    def build_nodes(self, rel_map, knowledge_sequence, depth=2) -> List[NodeWithScore]:
...
def test_simple_pruning(nebula_db: NebulaDB):
    ...
    question = (
        "Who were the top two contestants in the America's Got Talent Season 17 finale?"
    )
...
def test_ppr(db: NebulaDB):
    ...
    filter_rels, filter_triplets = filter_pr_rels(
        question, entities, triplets, rel_to_entities, max_ent=5
    )
