...
class DatasetManager:
    ...
    def _load_all_datasets(self):
        ...
        if not self.datasets_index_path.exists():
            logger.warning(f"Datasets index file not found: {self.datasets_index_path}")
            return
    ...
    def _load_graph_schemas(self, schema_file: Path, dataset_name: str):
        ...
        try:
            ...
            if len(schemas.datasets) > 1:
                print(
                    f"[WARNING] Graph schema file {schema_file} contains {len(schemas.datasets)} entries, expected 1. Using first entry."
                )
    ...
    def get_dataset_content(
        ...
        if dtype == "graph":
            ...
            # 大文件保护：超过 100MB 拒绝加载，建议使用流式读取
            file_size_mb = os.path.getsize(path) / (1024 * 1024)
            if file_size_mb > 100:
                raise ValueError(
                    f"文本文件过大 ({file_size_mb:.1f} MB)，超过 100MB 限制。"
                    " 建议使用流式读取或分块加载。"
                )
    ...
    def get_converted_graph_dataset(self, dataset_name: str) -> Optional[DatasetConfig]:
        ...
        try:
            schemas = load_data_upload_config(
                str(graph_schema_file), validate_files=False
            )
            if schemas.datasets:
    ...
    def load_graph_to_neo4j(
        self,
        graph_dataset_config: DatasetConfig,
        dataset_name: str,
        ...
        try:
            ...
            loader_schema = {"vertex": vertex_dicts, "edge": edge_dicts}

            logger.info(f"🔄 开始将图数据集 {dataset_name} 加载到Neo4j...")
            logger.info(
                f"   顶点配置数量: {len(vertex_dicts)}, 边配置数量: {len(edge_dicts)}"
            )
