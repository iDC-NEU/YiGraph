...
# 最小安全 builtins（禁止 __import__/open/eval/exec 等危险函数）
_MINIMAL_BUILTINS = {
    "len": len,
    "sorted": sorted,
    "reversed": reversed,
...
class DataDependencyResolver:
    ...
    def __init__(
        self, reasoner: Reasoner, error_recovery: Optional[ErrorRecovery] = None
    ):
    ...
    def set_global_graph(
        self, graph_nodes: List[str], graph_edges: List[Tuple[str, str]]
    ):
    ...
    async def resolve_dependencies(
        ...
        logger.info(
            f"📊 [依赖解析] 当前步骤:{step_id} | 上游节点:{len(data_dependency_parents)}"
        )
        ...
        if step.task_type == GraphAnalysisType.GRAPH_ALGORITHM:
            ...
            if graph_items:
                ...
                graph_input_adapter_result = conver_graph_llm_info.get(
                    "converted_graph"
                )
            ...
            if param_items:
                ...
                parameter_input_adapter_result = convert_parameter_llm_info.get(
                    "mapped_params"
                )
        ...
        if self.error_recovery:
            ...
            async def op_generate_and_execute_graph_code(error_history):
                ...
                if not fn or not callable(fn):
                    raise ValueError(
                        "Generated code does not define a valid transform_graph function."
                    )
            ...
            try:
                ...
                logger.info(
                    f"  Graph conversion (generate+execute) failed after retries: {e}"
                )
                return {
                    "description": str(e),
                    "converted_graph": None,
    ...
    async def _convert_parameter_dependencies(
        ...
        """
        if not param_items:
            return {"mapped_params": {}, "mapping_raw": None, "reasoning": None}
        ...
        if self.error_recovery:
            ...
            async def op_map_parameters_and_execute(error_history):
                ...
                if "mapping" not in mapping_result:
                    raise ValueError(
                        mapping_result.get(
                            "explanation", "LLM returned unexpected format"
                        )
                    )
                ...
                for param_name, mp in mapping.items():
                    ...
                    matched_item = next(
                        (
                            x
                            for x in param_items
                            if x.parent_step_id == parent_step_id
                            and x.field_key == from_field
                        ),
                        None,
                    )
